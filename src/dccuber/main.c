...
int main(int argc, char const *argv[])
{
  ...
  if (!fabrica_pid) // Solo el fabrica cumple el if
  {
    ...
    if (!repartidores_pid[0])
    {
      ...
      sprintf(&estado_semaforos[1], "%d", semaforos[1]);
      sprintf(&estado_semaforos[2], "%d", semaforos[2]);
      printf("ubicacion semaforo 1 %i\n", strtol(data_in->lines[0][0], NULL, 10));
      printf("ubicacion semaforo 2 %i\n", strtol(data_in->lines[0][1], NULL, 10));
      printf("ubicacion semaforo 3 %i\n", strtol(data_in->lines[0][2], NULL, 10));
      printf("ubicacion bodega %i\n", strtol(data_in->lines[0][3], NULL, 10));
      sprintf(&ubicacion_semaforos[0], "%d", strtol(data_in->lines[0][0], NULL, 10));
      sprintf(&ubicacion_semaforos[1], "%d", strtol(data_in->lines[0][1], NULL, 10));
      sprintf(&ubicacion_semaforos[2], "%d", strtol(data_in->lines[0][2], NULL, 10));
      sprintf(&ubicacion_bodega, "%d", strtol(data_in->lines[0][3], NULL, 10));
      printf("ubicacion semaforo 1 char %c\n", ubicacion_semaforos[0]);
      printf("ubicacion semaforo 2 char %c\n", ubicacion_semaforos[1]);
      printf("ubicacion semaforo 3 char %c\n", ubicacion_semaforos[2]);
      printf("ubicacion bodega char %c\n", ubicacion_semaforos[3]);
      myargs[0] = strdup("./repartidor");
